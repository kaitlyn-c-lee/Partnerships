...
library(pacman)
p_load(tidyverse, ggpubr, dplyr, stringr, readxl, pdftools, tm, writexl, pdfsearch, quanteda, NLP, wordcloud, wordcloud2, RColorBrewer, stargazer, texreg, weights, tidytext, wesanderson) 
...
# keep only non-duplicates in mission_statements (looks like there is one entity duplicate here)
...
social_theme <- themes$Socioeconomic
```
...
# T-test: Between level variation, by theme
* Do NGOs use more conservation theme language than non-NGOs?

* unweighted two tailed and one tailed are significant - NGOs use conservation themed language more than non-ngo on avgerage, stat sig at the 1% level.
* total area weighted - result changes sign, two tailed is stat sig but one tailed is not
* N PAs weighted  - same result as unweighted
  
  
  
* Do Local orgs use more community theme language than non-Local orgs?

* unweighted two tailed and one tailed are significant - local orgs use social themed language more than non-local on avgerage, stat sig at the 1% level.
* total area weighted - result changes sign, not stat sig
* N PAs weighted - same result as unweighted

```{r, warning=FALSE, message=FALSE}
##### NGO, Ecology theme #####
phrases_ecology$ngo <- if_else(str_detect(phrases_ecology$mngentity, "NGO"), 1, 0)

ecology_NGO <- phrases_ecology %>% filter(ngo==1)
ecology_nonNGO <- phrases_ecology %>% filter(ngo==0)

# Conduct t-test for difference in means (two-tailed)
# unweighted
t.test(ecology_NGO$freq, ecology_nonNGO$freq, alternative = c("two.sided"), mu = 0, conf.level = 0.95)
t.test(ecology_NGO$freq, ecology_nonNGO$freq, alternative = c("greater"), mu = 0, conf.level = 0.95)

# total area weighted
wtd.t.test(ecology_NGO$freq, ecology_nonNGO$freq, weight=ecology_NGO$tot_area, weighty=ecology_nonNGO$tot_area, alternative = c("two.tailed"))

wtd.t.test(ecology_NGO$freq, ecology_nonNGO$freq, weight=ecology_NGO$tot_area, weighty=ecology_nonNGO$tot_area, alternative = c("greater"))

# N PAs weighted
wtd.t.test(ecology_NGO$freq, ecology_nonNGO$freq, weight=ecology_NGO$units, weighty=ecology_nonNGO$units, alternative = c("two.tailed"))

wtd.t.test(ecology_NGO$freq, ecology_nonNGO$freq, weight=ecology_NGO$units, weighty=ecology_nonNGO$units, alternative = c("greater"))



##### Local, Social theme #####
phrases_social$loc <- if_else(str_detect(phrases_social$mngentity, "LOC"), 1, 0)

social_loc <- phrases_social %>% filter(loc==1)
social_nonloc <- phrases_social %>% filter(loc==0)

# Conduct t-test for difference in means (two-tailed)
# unweighted
t.test(social_loc$freq, social_nonloc$freq, alternative = c("two.sided"), mu = 0, conf.level = 0.95)
t.test(social_loc$freq, social_nonloc$freq, alternative = c("greater"), mu = 0, conf.level = 0.95)

# total area weighted
wtd.t.test(social_loc$freq, social_nonloc$freq, weight=social_loc$tot_area, weighty=social_nonloc$tot_area, alternative = c("two.tailed"))
wtd.t.test(social_loc$freq, social_nonloc$freq, weight=social_loc$tot_area, weighty=social_nonloc$tot_area, alternative = c("greater"))

# N PAs weighted
wtd.t.test(social_loc$freq, social_nonloc$freq, weight=social_loc$units, weighty=social_nonloc$units, alternative = c("two.tailed"))
wtd.t.test(social_loc$freq, social_nonloc$freq, weight=social_loc$units, weighty=social_nonloc$units, alternative = c("greater"))
...
social_ecology_plot <- ggplot(data, aes(fill=Status, y=Frequency, x=Theme)) + 
  ...
  coord_flip()
ggsave(social_ecology_plot, file="Output/Figure_4.png", width = 10, height = 5, dpi = 200)
social_ecology_plot
